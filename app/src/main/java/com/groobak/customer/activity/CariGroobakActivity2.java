...
public class CariGroobakActivity2 extends AppCompatActivity
    ...
    Context context = CariGroobakActivity2.this;
    ...
    private okhttp3.Callback updateRouteCallback = new okhttp3.Callback() {
        ...
        public void onResponse(@NonNull okhttp3.Call call, okhttp3.Response response) throws IOException {
            if (response.isSuccessful()) {
                ...
                final long distance = MapDirectionAPI.getDistance(CariGroobakActivity2.this, json);
                final String time = MapDirectionAPI.getTimeDistance(CariGroobakActivity2.this, json);
                if (distance >= 0) {
                    CariGroobakActivity2.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        namaikan = intent.getStringExtra("nama_ikan");
    ...
    private void updateLineDestination(String json) {
        Directions directions = new Directions(CariGroobakActivity2.this);
        try {
            ...
            if (routes.size() > 0) {
                directionLine = gMap.addPolyline((new PolylineOptions())
                        ...
                        .color(ContextCompat.getColor(CariGroobakActivity2.this, R.color.colorgradient))
                        .width(8));
    ...
    private void fetchNearDriver(double latitude, double longitude, String fitur) {
        ...
        if (lastKnownLocation != null) {
            ...
            param.setNama_ikan(namaikan);

            service.groobakperikan(param).enqueue(new Callback<GetNearRideCarResponseJson>() {
                @Override
                public void onResponse(@NonNull Call<GetNearRideCarResponseJson> call, @NonNull Response<GetNearRideCarResponseJson> response) {
                    if (response.isSuccessful()) {
                        ...
                        LinearLayoutManager layoutManager = new LinearLayoutManager(CariGroobakActivity2.this);
                ...
                public void run() {
                    ...
                    for (DriverModel cDriver : driverAvailable) {
                        if (cDriver.getId().equals(response.getId())) {
                            Intent intent = new Intent(CariGroobakActivity2.this, ProgressActivity.class);
