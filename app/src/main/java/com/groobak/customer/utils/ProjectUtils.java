...
public class ProjectUtils {
   ...
   /* public static double roundTwoDecimals(double d) {
    ...
    public static String generateSignInMobilePulsa(String sign, SettingPreference sp) {
        return sp.getSetting()[7] + sp.getSetting()[8] + sign;
    }
    ...
    public static List<String> getNext12MonthList() {
        Calendar calendar = Calendar.getInstance();
        List<String> allDates = new ArrayList<>();
        SimpleDateFormat monthDate = new SimpleDateFormat("MMM-yyyy", Locale.US);
        for (int i = 1; i <= 12; i++) {
            String monthName = "Bayar Hingga " + monthDate.format(calendar.getTime());
